...
UK_COMPANY_HEADERS = {"Company_name", "Region_short", "Accreditation_link"}
...
def load_existing_uk_cache() -> list[dict]:
    if not DATA_UK_PATH.exists():
        return []
    with open(DATA_UK_PATH) as f:
        return json.load(f)
...
def classify_sheet(title: str, rows: list[dict]) -> str | None:
    ...
    if UK_COMPANY_HEADERS.issubset(headers) and "uk" in title_normalized:
        return "uk_companies"
    if "usa" in title_normalized and "company_name" in {header.lower() for header in headers}:
        return "usa_companies"
    if "uk" in title_normalized and "company_name" in {header.lower() for header in headers}:
        return "uk_companies"
    return None
...
def log_uk_diagnostics(rows: list[dict]) -> None:
    companies = [
        {
            "Company_name": str(row.get("Company_name", "")).strip(),
            "Email": str(row.get("Email", "")).strip(),
    ...
    preview = companies[:5]
    print(
        f"UK diagnostics: rows={len(companies)}, rows_with_email={with_email}, "
        f"preview={preview}"
    )
...
def find_sheets(sh) -> dict[str, list[dict]]:
    ...
    manual_titles = {
        ...
        "uk_companies": (CACHE_UK_SHEET, "Company_name"),
    }
    ...
    for sheet_key, (sheet_title, primary_key) in manual_titles.items():
        ...
        if sheet_key == "uk_companies":
            log_uk_diagnostics(rows)
...
def refresh_cache():
    ...
    data_uk = sheets.get("uk_companies")
    if data_uk is None:
        data_uk = load_existing_uk_cache()
        print(f"UK worksheet not detected, keeping existing data_uk.json with {len(data_uk)} rows")

    save_cache_data(sheets["russia_companies"], sheets["russia_limits"], data_us, data_uk)
